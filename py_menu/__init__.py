  ...
  def __init__(self, header, options=None, splash="", ):
    ...
    for option in self.options:
      if not isinstance(option, Option):
        raise TypeError("'options' must be a list of Option objects")
      if isinstance(option.action, Menu):
        option.action.prev_menu = self.active_menu
      elif not hasattr(option.action, "__call__"):
        raise TypeError("The action of each option must either be a Menu or "\
                        "posess the __call__ attribute.")
    self.splash = splash
